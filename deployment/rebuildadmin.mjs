...
await $`ssh -o StrictHostKeyChecking=no -l ${username} ${ip_address} "rm -rf /var/www/luxelane-laravel/admin.zip /var/www/luxelane-laravel/admin";`
// let front_end_source_path = await question('Enter frontend.zip source path (ex: /home/../luxelane-laravel/frontend.zip): ')
let front_end_source_path = "./admin.zip";
echo("Uploading admin.zip to server, Please wait...")
await $`scp ${front_end_source_path} ${username}@${ip_address}:/var/www/luxelane-laravel`

echo(chalk.green("Uploaded admin.zip to server"))
await $`ssh -o StrictHostKeyChecking=no -l ${username} ${ip_address} "unzip /var/www/luxelane-laravel/admin.zip -d /var/www/luxelane-laravel";`

echo('Install Node For Admin')
await $`ssh -o StrictHostKeyChecking=no -l ${username} ${ip_address} "yarn --cwd /var/www/luxelane-laravel/admin/rest";`
