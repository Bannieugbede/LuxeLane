...
await $`yarn --cwd /var/www/luxelane-laravel/`;

echo(chalk.blue('Running For Shop App with pm2'))

await $`pm2 --name shop-rest start yarn --cwd /var/www/luxelane-laravel -- run start:shop-rest`;

echo(chalk.blue('Running For Admin App with pm2'))

await $`pm2 --name admin-rest start yarn --cwd /var/www/luxelane-laravel -- run start:admin-rest`;
