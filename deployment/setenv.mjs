...
await $`sudo rm -f /etc/nginx/sites-enabled/luxelane`
await $`sudo rm -f /etc/nginx/sites-available/luxelane`
await $`sudo touch /etc/nginx/sites-available/luxelane`
await $`sudo chmod -R 777 /etc/nginx/sites-available/luxelane`
...
await $`sudo echo 'server {
        ...
        # For API
        location /backend {
            alias /var/www/luxelane-laravel/luxelane-api/public;
    ...
    }' > '/etc/nginx/sites-available/luxelane'`;


echo(chalk.blue('\nEnabling the config'))
await $`sudo ln -s /etc/nginx/sites-available/luxelane /etc/nginx/sites-enabled/`
